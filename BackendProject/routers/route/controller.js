...
export const add = (req,res) =>{
    const {departureAirportId,arrivalAirportId,flightTime} = req.body
    pool.query(queries.checkRouteExist,[departureAirportId,arrivalAirportId],(error,results)=>{
        if(error) res.status(404).json({message:error.message})
        else if(results.rows.length) res.status(404).json({message:"Route already exists."})
        else {
            pool.query(queries.add,[departureAirportId,arrivalAirportId,flightTime],(error,results)=>{
                if(error) res.status(404).json({message:error.message})
